...
public sealed class DraftPickOrderConfiguration : IEntityTypeConfiguration<DraftPickOrder>
{
    public void Configure(EntityTypeBuilder<DraftPickOrder> builder)
    {
        ...
        builder.HasIndex(pick => new { pick.LeagueId, pick.OverallPick }).IsUnique();
        builder.HasIndex(pick => new { pick.LeagueId, pick.Round, pick.PositionInRound }).IsUnique();
        builder.HasIndex(pick => new { pick.LeagueId, pick.NbaPlayerId })
            .IsUnique()
