...
public sealed class DraftsController(
    ...
    IHubContext<FantasyLeagueHub> _hubContext,
    ILogger<DraftsController> _logger) : ControllerBase
{
    /// <summary>
    /// Returns the current draft state and 
    /// complete pick history for a league.
    /// </summary>
    /// <param name="leagueId">
    ...
    /// </param>
    /// <returns>
    /// The current draft state, active pick, and completed picks.
    /// </returns>
    /// <response code="200">
    /// The draft state was retrieved successfully.
    /// </response>
    /// <response code="404">
    ...
    /// <summary>
    /// Closes a league whose draft was delayed
    /// because minimum capacity was not reached.
    /// </summary>
    ...
    /// <response code="200">
    /// The delayed league was closed successfully.
    /// </response>
    /// <response code="403">
    /// The requester is not the league commissioner.
    /// </response>
    /// <response code="409">
    /// The league is not in the delayed state.
    /// </response>
    [HttpPost("close")]
    [ProducesResponseType<DraftStateResponse>(StatusCodes.Status200OK)]
    ...
    /// <response code="200">
    /// The player was drafted successfully.
    /// </response>
    /// <response code="403">
    /// The requester does not own the team.
    /// </response>
    /// <response code="404">
    /// The league, team, or NBA player was not found.
    /// </response>
    /// <response code="409">
    /// The draft is inactive, the turn is invalid, 
    /// or the player was already selected.
    /// </response>
    [HttpPost("picks")]
    [ProducesResponseType<DraftStateResponse>(StatusCodes.Status200OK)]
    [ProducesResponseType<ProblemDetails>(StatusCodes.Status403Forbidden)]
